...
export default function Home() {
  ...
  const [currentView, setCurrentView] = useState<'home' | 'learn' | 'profile' | 'search' | 'login'>('home');
  const [showGame, setShowGame] = useState(false);
  const [streakCount, setStreakCount] = useState(0);
  const [watchedVideos, setWatchedVideos] = useState<string[]>([]);
  const [gameScores, setGameScores] = useState<{videoId: string, score: number}[]>([]);
  ...
  // Handle navigation
  const handleNavigation = (view: 'home' | 'learn' | 'profile' | 'search' | 'login') => {
    setCurrentView(view);
